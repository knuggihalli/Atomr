...
// get variable sent from client-side page
  ...
  if (isset($_SESSION['Environment'])) {
    ...
    $url = "https://platform.boomi.com/api/rest/v1/". $_SESSION['account_id'] ."/EnvironmentAtomAttachment/" ;
    $idr = explode("*",$ids);
    $amountid = count($idr); 
    $x = 0; 
    $_SESSION['migrated_atoms'] = $_SESSION['val']; 

    for ($i=1; $i<$amountid; $i++) { 

      print_r($idr[$i]);

     $xml = '<bns:EnvironmentAtomAttachment atomId="'.$idr[$i].'" 
     environmentId="'. $_SESSION['envend'].'" xmlns:bns="http://api.platform.boomi.com/" 
